...
# ── Instagram auto-upload (optional) ──────────
AUTO_UPLOAD               = False                  # set True to upload after each video
INSTAGRAM_USER_ID         = "YOUR_IG_USER_ID"      # numeric ID — call /me?fields=id with your token
INSTAGRAM_ACCESS_TOKEN    = "YOUR_ACCESS_TOKEN"    # long-lived token from Meta Developer dashboard
...
SCRIPTS = [
   {
    "title": "Your brain rewards curiosity",
    "hook": "Curiosity activates the same reward systems as pleasure.",
    "story": "When your brain encounters unanswered questions, dopamine levels can rise as you search for the answer. That is why mysteries, cliffhangers, and strange facts feel deeply satisfying.",
    "outro": "Your brain is literally addicted to learning new things. Follow for more.",
    "hashtags": "#psychology #brainfacts #science #shorts #fyp",
    "keywords": [
        "women mystery cinematic",
        "brain dopamine visualization",
        "woman curiosity aesthetic",
        "question mark woman",
        "woman learning dark visuals"
    ],
    "music": "cinematic"
}
]
...
def upload_to_cloudinary(local_path: str) -> Optional[str]:
...
def upload_to_instagram(video_path: str, script: dict) -> bool:
    """
    Upload a video to Instagram as a Reel using the Graph API.
    Flow: S3 upload → create container → poll until ready → publish
    """
    import time

    if INSTAGRAM_USER_ID == "YOUR_IG_USER_ID" or INSTAGRAM_ACCESS_TOKEN == "YOUR_ACCESS_TOKEN":
        print("  Instagram credentials not set — skipping upload.")
        return False

    # ── Step 1: host the video at a public URL ──
    video_url = upload_to_cloudinary(video_path)
    if not video_url:
        return False

    base      = f"https://graph.facebook.com/v19.0/{INSTAGRAM_USER_ID}"
    auth      = {"access_token": INSTAGRAM_ACCESS_TOKEN}
    ...
    try:
        ...
        print(f"  Failed to create IG container: {e} — {r.text if 'r' in dir() else ''}")
        return False
    ...
    for _ in range(24):   # up to ~2 minutes
        ...
        try:
            s = requests.get(
                ...
                params={**auth, "fields": "status_code"},
                timeout=15,
            )
        ...
        print("\n  Timed out waiting for Instagram to process.")
        return False
    print()
    ...
    try:
        ...
        p.raise_for_status()
        print(f"  Reel published! Media ID: {p.json()['id']}")
        return True
    except Exception as e:
        print(f"  Publish failed: {e} — {p.text if 'p' in dir() else ''}")
        return False
...
def main():
    ...
    for i in range(SHORTS_PER_DAY):
        ...
        try:
            ...
            made.append(path)
            if AUTO_UPLOAD:
                upload_to_instagram(path, script)
        except Exception as e:
