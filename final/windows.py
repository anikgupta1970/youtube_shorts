...
SCRIPTS = [
{
  "title": "India once had forts designed like giant military puzzles",
  "hook": "Some invading armies reportedly became trapped before battles even started.",
  "story": "Several Indian forts were engineered with confusing pathways, hidden entrances, angled gates, steep staircases, and maze-like layouts specifically designed to slow invading armies. Forts across Rajasthan and Maharashtra often forced enemies to move through narrow turns while defenders attacked from elevated positions above. Some routes were intentionally built to confuse unfamiliar attackers, making direct invasions extremely difficult. Combined with massive walls, secret escape routes, and natural hilltop locations, these forts became powerful examples of strategic military architecture in Indian history.",
  "outro": "Sometimes intelligence was more powerful than weapons. Follow for more unbelievable Indian history.",
  "tags": ["#india", "#history", "#fort", "#war"],
  "hashtags": "#india #history #fort #war #shorts #fyp",
  "keywords": [
    "indian fort cinematic",
    "ancient battlefield visuals",
    "historic architecture aesthetic",
    "military documentary montage"
  ],
  "music": "suspense"
}
]
...
def get_pexels_videos(keywords: list, count: int = 4, shuffle: bool = False) -> list:
    """
    Fetch exactly one clip per keyword slot (up to `count` slots).
    Returns a list of length min(count, len(keywords)) where each entry is
    a file path or None — position[i] always corresponds to keyword[i] so
    a failed slot never shifts subsequent clips into the wrong section.
    """
    ...
    for i, keyword in enumerate(slots):
        ...
        found = False
        for v in videos:
            if v.get("id") in used_ids or v.get("duration", 0) < 8:
                continue
...
def find_section_boundaries(all_words: list, script: dict) -> tuple:
    ...
    def find_cue(cue_text):
        cue_words = [norm(w) for w in cue_text.split()[:4] if norm(w)]
...
def make_short(script: dict, index: int) -> str:
    ...
    # ── Background video ──
    bg_paths        = get_pexels_videos(script["keywords"], count=4, shuffle=False)
    ...
    section_durs = [
        max(MIN_CLIP, story_start               + _buf),
        max(MIN_CLIP, _story_mid - story_start  + _buf),
        max(MIN_CLIP, outro_start - _story_mid  + _buf),
        max(MIN_CLIP, narr_dur   - outro_start  + _buf),
    ]
    ...
    def safe_close(clip):
        """
        Recursively close MoviePy clips and readers.
        Prevents WinError 32 on Windows.
        """
    ...
    print("  Closing video/audio handles...")
    ...
    # EXTRA: close nested source clips
    if 'bg_paths' in locals():
    ...
    temp_files = ["_tmp_voice.mp3"]

    for i in range(4):
        temp_files.append(f"_tmp_bg_{i}.mp4")
